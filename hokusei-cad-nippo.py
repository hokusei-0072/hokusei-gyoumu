# 2025/09/11 編集　メーカー名にインフェックを追加、作業内容で見積を選択した場合、内容を自動入力。
...
if name != '選択してください':
    ...
    def create_input_fields(index):
        ...
        customer = st.selectbox(
            f'メーカー{index}',
            ('選択してください', 'ジーテクト', 'ヨロズ', '城山', 'タチバナ', '浜岳', '三池', '東プレ', '東海鉄工所', '坪山',
             'インフェック', '千代田', '雑務', 'その他メーカー'),
        ...
        if genre != '見積':
            ...
            number = st.text_input(f'工番を入力{index}', key=f'number_{index}',
                                   value="見積用造形、解析").upper() if genre != '選択してください' else ''
